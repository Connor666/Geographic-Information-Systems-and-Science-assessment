...
map <- 
  ...
  geom_point(data=myData, aes(x=longitude, y = latitude, size=open.rate, color=open.rate, alpha=open.rate,text=paste("City: ",province,"\n","Opening rate: ",round(open.rate,2))), shape=20, stroke=FALSE) +
...
bubblefunc <- function(myData,size,name,myBreaks){
  ...
  map <- 
    ...
    geom_point(data=myData, aes(x=longitude, y = latitude, size=size, color=size, alpha=size,text=paste("City: ",province,"\n","Coefficient: ",round(size,4))), shape=20, stroke=FALSE) +
